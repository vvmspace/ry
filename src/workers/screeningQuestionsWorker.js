...
async function claimNextJobForScreeningQuestions() {
  const job = await JobPage.findOneAndUpdate(
    {
      status: { $in: ['screening', 'interview'] },
      $or: [
        { screeningQuestionsAnswers: null },
        { screeningQuestionsAnswers: "" },
        { screeningQuestionsAnswers: { $exists: false } }
      ],
      $or: [
        { screeningQuestionsAnswersStartedAt: { $exists: false } },
        { screeningQuestionsAnswersStartedAt: { $lt: new Date(Date.now() - 10 * 60 * 1000) } } // 10 min timeout
      ]
    },
    { $set: { screeningQuestionsAnswersStartedAt: new Date() } },
    { returnDocument: 'after', sort: { screeningQuestionsAnswers: -1, createdAt: -1 } }
  );

  return { job, noMoreJobs: !job && !(await JobPage.exists({
    status: { $in: ['screening', 'interview'] },
    $or: [{ screeningQuestionsAnswers: null }, { screeningQuestionsAnswers: "" }, { screeningQuestionsAnswers: { $exists: false } }]
  })) };
}

async function generateScreeningQuestions(jobId) {
  ...
  const promptPath = path.resolve(process.cwd(), 'prompts/screening_questions.md');
  ...
  if (result) {
    job.screeningQuestionsAnswers = result;
    job.screeningQuestionsAnswersStartedAt = undefined;
...
async function runScreeningQuestionsWorker() {
  console.log('\x1b[36m\x1b[1m❓  SCREENING QUESTIONS WORKER\x1b[0m');
  ...
  try {
    while (true) {
      let job;
      try {
        const result = await claimNextJobForScreeningQuestions();
        if (result.noMoreJobs) {
          console.log(`No more jobs found for screening questions generation.`);
          break;
        }
        if (!result.job) {
          continue;
      ...
      console.log(`Generating screening questions answers for job: ${job.title} (${job.companyName})`);

      try {
        await generateScreeningQuestions(job._id);
        console.log(`✅ Successfully generated screening questions answers for ${job.url}`);
      } catch (err) {
        console.error(`Screening questions generation failed for ${job.url}:`, err);
        job.screeningQuestionsAnswersStartedAt = undefined; // Clear lock to allow retry
        await job.save();
      }
    }
  } catch (err) {
    console.error("Screening Questions Worker fatal error:", err);
    process.exitCode = 1;
  } finally {
...
if (require.main === module) {
  ...
  runScreeningQuestionsWorker().catch(err => {
    console.error(err);
    process.exit(1);
  }).finally(() => {
...
module.exports = { runScreeningQuestionsWorker, generateScreeningQuestions };
