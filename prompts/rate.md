...
Add +5% to match_rate if candidate is over-qualified.
...
6. -5% if the company is from the USA and relocation is required to the USA.
...
`{ "match_rate": 87, "comment": "up to 10 words", "icon": "try to explain in one emoji" }`
